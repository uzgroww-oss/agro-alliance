...
export async function loadBloggers(params?: {
  ...
  try {
    ...
    if (params?.region && params.region !== "Barchasi") qs.set("region", params.region)
    if (params?.search) qs.set("search", params.search)
    if (params?.sort === "Reyting bo'yicha") qs.set("sort", "rating")
    else if (params?.sort === "Obunachilar bo'yicha") qs.set("sort", "subscribers")

    const res = await api<BloggerListResponse>(`/public/bloggers?${qs.toString()}`)
    return res
  } catch (err) {
