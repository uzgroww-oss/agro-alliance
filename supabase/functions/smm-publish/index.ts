    ...
    const loadConnections = async () => {
      ...
      for (const c of (conns || []) as Array<{ platform: string; config: Record<string, unknown>; display_name: string | null }>) {
        ...
        const ok =
          ...
          /**
           * YouTube uchun kanal ID YETARLI EMAS.
           *
           * Ilgari kanal havolasi bo'yicha "ulangan" deb ko'rsatilardi,
           * lekin u faqat ochiq statistikani o'qishga yarardi. Video
           * yuklash / tahrirlash uchun Google roziligi shart. Ikkalasi
           * bir xil ko'rinsa, foydalanuvchi "ulangan" deb turgan
           * kanalda studiya ishlamasligini tushunmasdi.
           */
          c.platform === "youtube" ? Boolean(cfg.refresh_token || cfg.access_token) :
