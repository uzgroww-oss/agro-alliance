...
// me-socials — bloger ijtimoiy tarmoqlari (POST=qo'shish, DELETE=o'chirish)
// me-socials-add + me-socials-delete birlashtirilgan (funksiya slotini tejash uchun)
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      if (!platform) return errorResponse("Platformani aniqlab bo'lmadi", 400)
      ...
      if (existing) return errorResponse("Bu platforma allaqachon qo'shilgan", 409)

      const accountName = extractName(body.link) || platform.name
      const { error: insertError } = await supabaseAdmin
        .from("social_accounts").insert({
    ...
    if (req.method === "DELETE") {
      ...
      const { data: account } = await supabaseAdmin
        .from("social_accounts").select("id")
        .eq("id", id).eq("blogger_id", auth.user.id).is("deleted_at", null).maybeSingle()
      if (!account) return errorResponse("Ijtimoiy tarmoq topilmadi", 404)

      const { error: deleteError } = await supabaseAdmin
        .from("social_accounts").update({ deleted_at: now(), deleted_by: auth.user.id }).eq("id", id)
      if (deleteError) return errorResponse(deleteError.message, 500)

      await supabaseAdmin
        .from("social_statistics").update({ deleted_at: now(), deleted_by: auth.user.id }).eq("account_id", id)
      return jsonResponse({ success: true })
    }
